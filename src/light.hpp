...
public:
    ...
    Light(LightType type, glm::vec3 position = glm::vec3(0.0f), glm::vec3 direction = glm::vec3(0.0f, -1.0f, 0.0f), glm::vec3 color = glm::vec3(1.0f), float intensity = 1.0f)
    ...
    void generateShadowMap() {
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "Erreur de framebuffer pour la shadow map!" << std::endl;
        }

        if (type == DIRECTIONAL) {
            lightSpaceMatrix = glm::ortho(-50.0f, 50.0f, -50.0f, 50.0f, 0.1f, 100.0f) * glm::lookAt(position, position + direction, glm::vec3(0.0f, 1.0f, 0.0f));
        } else if (type == SPOT) {
            lightSpaceMatrix = glm::perspective(glm::radians(45.0f), 1.0f, 0.1f, 100.0f) * glm::lookAt(position, position + direction, glm::vec3(0.0f, 1.0f, 0.0f));
        }
    }
